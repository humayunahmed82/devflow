...
const Theme = () => {
  ...
  return (
    <Menubar className="relative border-none bg-transparent shadow-none">
      <MenubarMenu>
        <MenubarTrigger className="active-theme  data-[state-open]:bg-light-900  dark:data-[state-open]:bg-dark-200">
          {mode === "light" ? (
            <Image
        ...
        <MenubarContent className="absolute right-[-3rem] mt-3 min-w-[120px] rounded border bg-light-900 py-2 dark:border-dark-400 dark:bg-dark-300">
          {themes.map((item) => (
            <MenubarItem
              ...
              className="flex items-center gap-4 px-2.5 py-2 dark:bg-dark-400"
            >
              <Image
                src={item.icon}
                height={16}
